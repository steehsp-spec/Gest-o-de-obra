  ...
  return (
    <div className="p-4 lg:p-8 pb-24 lg:pb-8 max-w-4xl mx-auto">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-xl lg:text-2xl font-bold text-white">Configurações do Sistema</h1>
          <p className="text-gray-400 text-xs lg:text-sm">Gerencie as preferências da A&R Engenharia</p>
        </div>
        {message && (
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-[#161B22] rounded-2xl border border-white/10 overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-white/10 bg-white/5 flex items-center gap-3">
          ...
          <div className="p-4 lg:p-6 grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
            <div className="space-y-2">
              ...
              <input 
                ...
                className="w-full bg-[#0B0E14] border border-white/10 rounded-lg p-3 text-white focus:outline-none focus:border-[#F97316] text-sm"
              />
            </div>
            <div className="space-y-2">
        ...
        <div className="bg-[#161B22] rounded-2xl border border-white/10 overflow-hidden">
          ...
          <div className="p-4 lg:p-6 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <h4 className="text-white font-medium text-sm lg:text-base">Tema do Sistema</h4>
                <p className="text-xs lg:text-sm text-gray-500">Escolha a aparência da interface</p>
              </div>
              <select 
                value={formData.theme || 'dark'}
                onChange={(e) => setFormData({ ...formData, theme: e.target.value as any })}
                className="bg-[#0B0E14] border border-white/10 rounded-lg px-4 py-2 text-white text-sm focus:outline-none focus:border-[#F97316] w-full sm:w-auto"
              >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
              <div>
                <h4 className="text-white font-medium text-sm lg:text-base">Idioma</h4>
                <p className="text-xs lg:text-sm text-gray-500">Idioma padrão das labels e relatórios</p>
              </div>
              <select 
            ...
            <div className="flex items-center justify-between">
              <div>
                <h4 className="text-white font-medium text-sm lg:text-base">Notificações por E-mail</h4>
                <p className="text-xs lg:text-sm text-gray-500">Receber alertas de pendências críticas</p>
              </div>
        ...
        <div className="bg-[#161B22] rounded-2xl border border-white/10 overflow-hidden">
          <div className="p-4 lg:p-6 border-b border-white/10 bg-white/5 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs lg:text-sm text-gray-500 mt-1">Exporte e restaure os dados do sistema com segurança.</p>
            </div>
          </div>
          <div className="p-4 lg:p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
              {/* Export */}
              <div className="bg-[#0B0E14] rounded-xl p-4 lg:p-5 border border-white/5 space-y-4 flex flex-col justify-center">
                <button 
                  type="button"
              ...
              {/* Import */}
              <div className="bg-[#0B0E14] rounded-xl p-4 lg:p-5 border border-white/5 space-y-4">
                <div className="space-y-4">
                  <input 
                    ...
                    className="w-full text-[10px] lg:text-xs text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-[10px] lg:text-xs file:font-semibold file:bg-white/5 file:text-white hover:file:bg-white/10 cursor-pointer"
                  />
                  ...
                  <div className="flex items-center gap-4">
                    <label className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className="text-[10px] lg:text-xs text-gray-300">Mesclar dados</span>
                    </label>
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input 
                        type="radio" 
                        ...
                        className="text-[#F97316] focus:ring-[#F97316] bg-[#0B0E14] border-white/10"
                      />
                      <span className="text-[10px] lg:text-xs text-gray-300">Substituir tudo</span>
                    </label>
                  </div>
            ...
            {importMode === 'replace' && (
              <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-lg flex items-start gap-3">
                ...
                <p className="text-[10px] lg:text-[11px] text-red-400 leading-relaxed">
          ...
          <div className="bg-[#161B22] rounded-2xl border border-white/10 overflow-hidden">
            ...
            <div className="p-4 lg:p-6 space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h4 className="text-white font-medium text-sm lg:text-base">Migração de Dados</h4>
                  <p className="text-xs lg:text-sm text-gray-500">Sincronizar dados locais (localStorage) com o Firestore</p>
                </div>
                <button 
                  type="button"
                  onClick={handleMigrate}
                  disabled={isMigrating}
                  className="bg-white/5 hover:bg-white/10 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-colors border border-white/10 disabled:opacity-50 w-full sm:w-auto"
                >
              ...
              <div className="bg-[#0B0E14] rounded-xl p-4 border border-white/5">
                <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3">Resumo da Auditoria de Dados</h4>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {Object.entries(dataStatus.counts).map(([key, count]) => (
                    <div key={key} className="flex flex-col">
                      ...
                      <span className="text-base lg:text-lg font-bold text-white">{count}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-8 pt-8 border-t border-red-500/20">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h4 className="text-red-500 font-bold mb-1 text-sm lg:text-base">Zona de Perigo</h4>
                    <p className="text-xs lg:text-sm text-gray-500">Ações destrutivas que não podem ser desfeitas.</p>
                  </div>
                  <button 
                    type="button"
                    onClick={handleClearAllData}
                    className="bg-red-500/10 hover:bg-red-500/20 text-red-500 px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-colors border border-red-500/20 font-semibold w-full sm:w-auto"
                  >
                    <AlertCircle size={18} />
              ...
              {migrationLog.length > 0 && (
                <div className="bg-[#0B0E14] rounded-xl p-4 border border-white/5 max-h-40 overflow-y-auto">
                  <h4 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Logs de Migração</h4>
                  <ul className="space-y-1">
                    {migrationLog.map((log, i) => (
                      <li key={i} className="text-[10px] text-emerald-500 flex items-center gap-2">
              ...
              <div className="pt-6 border-t border-white/10">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <h4 className="text-white font-medium text-sm lg:text-base">Recalcular Dados</h4>
                    <p className="text-xs lg:text-sm text-gray-500">Recalcular todos os progressos e pesos do dashboard e cronogramas</p>
                  </div>
                  <button 
                    type="button"
                    onClick={async () => {
                    ...
                    className="bg-orange-500/10 hover:bg-orange-500/20 text-orange-500 px-4 py-2 rounded-lg flex items-center justify-center gap-2 transition-colors border border-orange-500/20 font-bold text-sm w-full sm:w-auto"
                  >
                    <RefreshCw size={18} />
                    Recalcular Agora
                  </button>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-end gap-4 pt-4">
          <button 
            type="button"
            className="px-6 py-2 text-gray-400 hover:text-white transition-colors w-full sm:w-auto"
          >
          ...
          <button 
            type="submit"
            disabled={isSaving}
            className="bg-[#F97316] hover:bg-[#EA580C] disabled:opacity-50 disabled:cursor-not-allowed text-white px-8 py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all hover:scale-105 active:scale-95 shadow-lg shadow-[#F97316]/20 w-full sm:w-auto"
          >
            <Save size={20} />
