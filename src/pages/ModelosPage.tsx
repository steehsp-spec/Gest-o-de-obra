    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          <h1 className="text-xl lg:text-3xl font-bold text-white flex items-center gap-2 lg:gap-3">
            <Layers className="text-[#F97316] lg:w-7 lg:h-7" size={24} />
            Modelos de Obra
          </h1>
          <p className="text-gray-400 text-xs lg:text-sm mt-1">Gerencie os modelos de cronograma padrão da empresa</p>
        </div>
        <button
          onClick={() => handleOpenModal()}
          className="bg-[#F97316] hover:bg-[#EA580C] text-white px-4 lg:px-6 py-2.5 lg:py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition-all hover:scale-105 active:scale-95 shadow-lg shadow-[#F97316]/20 text-sm lg:text-base"
        >
          <Plus size={20} />
          Novo Modelo
        </button>
      ...
      {message && (
        <motion.div 
          ...
          className={`p-3 lg:p-4 rounded-xl mb-6 flex items-center gap-3 text-sm lg:text-base ${
      ...
      <div className="relative mb-8">
        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 lg:w-5 lg:h-5" size={18} />
        <input
          type="text"
          placeholder="Buscar modelos..."
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          className="w-full bg-[#161B22] border border-white/10 rounded-xl py-2.5 lg:py-3 pl-12 pr-4 text-white text-sm lg:text-base focus:outline-none focus:border-[#F97316] transition-colors"
        />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
        {filteredTemplates.map((template) => {
          ...
          return (
            <motion.div
              ...
              className="bg-[#161B22] border border-white/10 rounded-2xl p-5 lg:p-6 hover:border-[#F97316]/50 transition-all group relative overflow-hidden"
            >
              <div className="flex justify-between items-start mb-4">
                <div className="bg-[#F97316]/10 p-2.5 lg:p-3 rounded-xl">
                  <Layers className="text-[#F97316] lg:w-6 lg:h-6" size={20} />
                </div>
              ...
              <h3 className="text-lg lg:text-xl font-bold text-white mb-2">{template.name}</h3>
              ...
              <button
                onClick={() => handleOpenModal(template)}
                className="w-full py-2.5 lg:py-3 bg-white/5 hover:bg-white/10 text-white rounded-xl font-medium transition-colors flex items-center justify-center gap-2 text-sm lg:text-base"
              >
        ...
        {filteredTemplates.length === 0 && (
          <div className="col-span-full py-12 lg:py-20 text-center">
            <Layers className="mx-auto text-gray-600 mb-4 lg:w-12 lg:h-12" size={40} />
            <p className="text-gray-400 text-base lg:text-lg">Nenhum modelo encontrado.</p>
          </div>
        )}
      </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-[#0B0E14] border-t sm:border border-white/10 rounded-t-2xl sm:rounded-2xl w-full max-w-4xl h-full sm:h-auto sm:max-h-[90vh] flex flex-col shadow-2xl"
            >
              <div className="p-4 lg:p-6 border-b border-white/10 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-400 text-[10px] lg:text-sm mt-0.5 lg:mt-1">Defina a estrutura padrão para este modelo</p>
                </div>
                <button
              ...
              <div className="p-4 lg:p-6 flex-1 overflow-y-auto custom-scrollbar space-y-6 lg:space-y-8">
                {/* Nome do Modelo */}
                <div>
                  <label className="block text-xs lg:text-sm font-medium text-gray-400 mb-1.5 lg:mb-2">Nome do Modelo</label>
                  <input
                    type="text"
                    value={formData.name}
                    onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))}
                    placeholder="Ex: Obra de Alto Padrão"
                    className="w-full bg-[#161B22] border border-white/10 rounded-xl py-2.5 lg:py-3 px-4 text-white text-sm lg:text-base focus:outline-none focus:border-[#F97316] transition-colors"
                  />
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-base lg:text-lg font-bold text-white flex items-center gap-2">
                      ...
                      <span className="text-[10px] lg:text-xs font-normal text-gray-500 bg-white/5 px-2 py-0.5 lg:py-1 rounded-full">
                        {formData.structure?.length || 0} Etapas
                      </span>
                    </h3>
                    <button
                      onClick={addStep}
                      className="text-[#F97316] hover:text-[#EA580C] text-xs lg:text-sm font-bold flex items-center gap-1 px-2.5 lg:px-3 py-1.5 rounded-lg hover:bg-[#F97316]/10 transition-colors"
                    >
                  ...
                  <div className="space-y-4">
                    {formData.structure?.map((step, index) => (
                      <div key={step.id} className="bg-[#161B22] border border-white/10 rounded-xl overflow-hidden">
                        <div className="p-3 lg:p-4 flex flex-col sm:flex-row sm:items-center gap-3 lg:gap-4 bg-white/5 border-b border-white/5">
                          <div className="flex items-center justify-between sm:justify-start gap-2">
                            ...
                            <button
                              ...
                              className="sm:hidden p-2 text-gray-500 hover:text-red-500 transition-colors"
                            >
                              <Trash2 size={18} />
                            </button>
                          </div>
                          ...
                          <input
                            ...
                            className="flex-1 bg-transparent border-none text-white font-bold focus:ring-0 p-0 text-base lg:text-lg"
                            placeholder="Nome da Etapa"
                          />

                          <div className="flex items-center justify-between sm:justify-end gap-4">
                            <div className="flex items-center gap-2">
                              <span className="text-[9px] lg:text-[10px] uppercase text-gray-500 font-bold">Peso (%)</span>
                              <input
                                type="number"
                                value={step.weight}
                                onChange={(e) => updateStep(step.id, { weight: Number(e.target.value) })}
                                className="w-14 lg:w-16 bg-[#0B0E14] border border-white/10 rounded px-2 py-1 text-white text-xs lg:text-sm text-center"
                              />
                            </div>
                            <button
                              onClick={() => removeStep(step.id)}
                        ...
                        <div className="p-3 lg:p-4 space-y-2">
                          {step.subSteps.map((sub, subIndex) => (
                            <div key={sub.id} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 pl-4 sm:pl-8 group">
                              <div className="flex items-center justify-between sm:justify-start gap-2">
                                ...
                                <button
                                  ...
                                  className="sm:hidden p-1 text-gray-600 hover:text-red-500"
                                >
                                  <X size={14} />
                                </button>
                              </div>
                              ...
                              <input
                                ...
                                className="flex-1 bg-transparent border-b border-white/5 text-gray-300 text-xs lg:text-sm focus:border-[#F97316] focus:outline-none py-1"
                              ...
                              <div className="flex items-center justify-between sm:justify-end gap-2">
                                <select
                                  ...
                                  className="bg-[#0B0E14] border border-white/10 rounded px-2 py-1 text-[10px] lg:text-xs text-gray-400"
                                >
                                ...
                                <button
                                  onClick={() => removeSubStep(step.id, sub.id)}
                                  className="hidden sm:block p-1 text-gray-600 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"
                                >
                                  <X size={14} />
                                </button>
                              </div>
                          ...
                          <button
                            ...
                            className="flex items-center gap-2 text-[10px] lg:text-xs text-gray-500 hover:text-[#F97316] transition-colors pl-4 sm:pl-8 mt-2"
                          >
                            <Plus size={14} />
                            Adicionar Subitem
                          </button>
                    ...
                    {(!formData.structure || formData.structure.length === 0) && (
                      <div className="text-center py-8 lg:py-12 border-2 border-dashed border-white/5 rounded-2xl">
                        <Layers className="mx-auto text-gray-700 mb-3 lg:w-8 lg:h-8" size={28} />
                        <p className="text-gray-500 text-xs lg:text-sm">Nenhuma etapa definida. Comece adicionando uma etapa principal.</p>
                        <button
                          onClick={addStep}
                          className="mt-4 text-[#F97316] hover:underline font-bold text-xs lg:text-sm"
                        >
              ...
              <div className="p-4 lg:p-6 border-t border-white/10 flex items-center justify-end gap-3 lg:gap-4 bg-white/5 sm:rounded-b-2xl">
                <button
                  onClick={handleCloseModal}
                  className="px-4 lg:px-6 py-2 lg:py-2.5 text-gray-400 hover:text-white text-sm lg:text-base font-medium transition-colors"
                >
                  Cancelar
                </button>
                <button
                  onClick={handleSave}
                  disabled={isSaving}
                  className="bg-[#F97316] hover:bg-[#EA580C] disabled:opacity-50 text-white px-6 lg:px-8 py-2 lg:py-2.5 rounded-xl font-bold flex items-center gap-2 transition-all shadow-lg shadow-[#F97316]/20 text-sm lg:text-base"
                >
