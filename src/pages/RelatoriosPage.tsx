    ...
    return filteredProjects.map(project => {
      ...
      return {
        ...
        stats: {
          ...
          openPendencies: projectPendencies.filter(p => p.status === 'aberta' || p.status === 'em_analise').length,
          income: totalIncome,
    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 text-sm">Análise de desempenho e saúde das obras</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-[#161B22] px-4 py-2 rounded-lg border border-white/10">
          ...
          <div key={data.id} className="bg-[#161B22] rounded-2xl border border-white/10 overflow-hidden">
            <div className="p-6 border-b border-white/10 bg-white/5 flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-gray-400">{data.client} • {data.location}</p>
              </div>
              <div className="text-right">
                <span className="text-xs text-gray-500 uppercase font-bold tracking-widest block mb-1">Status</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 divide-x divide-white/5">
              <div className="p-6">
                <div className="flex items-center gap-3 mb-4 text-blue-400">
                  <TrendingUp size={20} />
                  <span className="text-sm font-semibold uppercase tracking-wider">Progresso Físico</span>
                </div>
                <div className="flex items-end gap-2 mb-2">
                  <span className="text-3xl font-bold text-white">{data.stats.progress}%</span>
                  <span className="text-gray-500 text-xs mb-1">Concluído</span>
                </div>
              ...
              <div className="p-6">
                <div className="flex items-center gap-3 mb-4 text-[#F97316]">
                  <FileText size={20} />
                  <span className="text-sm font-semibold uppercase tracking-wider">Atividades</span>
                </div>
                <div className="flex items-end gap-2 mb-1">
                  <span className="text-3xl font-bold text-white">{data.stats.completedActivities}</span>
                  <span className="text-gray-500 text-xs mb-1">de {data.stats.activitiesCount} concluídas</span>
                </div>
                <p className="text-xs text-gray-400">Taxa de conclusão: {data.stats.activitiesCount > 0 ? Math.round((data.stats.completedActivities / data.stats.activitiesCount) * 100) : 0}%</p>
              </div>

              <div className="p-6">
                <div className="flex items-center gap-3 mb-4 text-red-400">
                  <AlertCircle size={20} />
                  <span className="text-sm font-semibold uppercase tracking-wider">Pendências</span>
                </div>
                <div className="flex items-end gap-2 mb-1">
                  <span className="text-3xl font-bold text-white">{data.stats.openPendencies}</span>
                  <span className="text-gray-500 text-xs mb-1">críticas / abertas</span>
                </div>
                <p className="text-xs text-gray-400">Total de pendências registradas: {data.stats.pendenciesCount}</p>
              </div>

              <div className="p-6">
                <div className="flex items-center gap-3 mb-4 text-emerald-400">
                  <DollarSign size={20} />
                  <span className="text-sm font-semibold uppercase tracking-wider">Financeiro</span>
                </div>
                <div className="flex items-end gap-2 mb-1">
                  <span className={`text-2xl font-bold ${data.stats.balance >= 0 ? 'text-emerald-500' : 'text-red-500'}`}>
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(data.stats.balance)}
                  </span>
                </div>
